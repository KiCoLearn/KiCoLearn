...
public abstract class HttpConnection {
    ...
    // get방식 rest 호출시 사용
    public static StringBuilder get(String apiURL) throws IOException {
    ...
    // post방식 rest 호출시 사용
    public static StringBuilder post(String apiURL, Map<String, String> params) throws IOException {
        URL url = new URL(apiURL);
        HttpURLConnection con = (HttpURLConnection) url.openConnection();
    ...
    // 서버에 요청하는 메소드
    public static StringBuilder responseHttp(HttpURLConnection con) throws IOException {
        StringBuilder response = new StringBuilder();

        int responseCode = con.getResponseCode();
        BufferedReader br;
        if (200 <= responseCode && responseCode < 300) { // 정상 호출
